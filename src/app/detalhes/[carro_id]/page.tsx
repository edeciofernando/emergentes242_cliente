...
export default function Detalhes() {
  ...
  useEffect(() => {
    async function buscaDados() {
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/carros/${carro_id}`)
    ...
    async function buscaFotos() {
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/fotos/${carro_id}`)
      const dados = await response.json()
      setFotos(dados)
    }
    buscaFotos()
